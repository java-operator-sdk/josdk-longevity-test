...
public class ConfigMapReconciler implements Reconciler<ConfigMap>, EventSourceInitializer<ConfigMap> {
...
//        });
    ...
    public Map<String, EventSource> prepareEventSources(EventSourceContext<ConfigMap> context) {
        InformerEventSource<Secret, ConfigMap> secretES = new InformerEventSource<>(InformerConfiguration.from(Secret.class,context)
                .build(), context);
